...
func RemoveDuplicatePods(
	...
	for ownerKey, podNodes := range duplicatePods {
		...
		if int(ownerKeyOccurence[ownerKey]) > nodeCountWithoutTaints {
			klog.V(2).InfoS("[Skipping Eviction] The number of replicas is more than the number of nodes", "pod", ownerKey.name, "replicaNumber", float64(ownerKeyOccurence[ownerKey]), "nodeCountWithoutTaints", nodeCountWithoutTaints)
		 	continue
		} else {
			klog.V(2).InfoS("[Start Eviction] The number of replicas is less (or the same) as the number of nodes", "pod", ownerKey.name, "replicaNumber", float64(ownerKeyOccurence[ownerKey]), "nodeCountWithoutTaints", nodeCountWithoutTaints)
		}
